...
class BatchSimulationTracker:
    ...
    def __init__(self, profit_percentage: float = 0.20):
        self.results = []
        self.profit_percentage = profit_percentage  # Configurable profit percentage metric
    ...
    def generate_cost_benefit_graphs(self, output_dir="results"):
        """Generates Cost-Benefit Analysis visualizations based on batch simulation data."""
        if not self.results:
            return
        ...
        # --- 1. Scatter Plot: Cost vs Net Profit ---
        costs = [r["cost"] for r in self.results]
        profits = [r["net_profit"] for r in self.results]
        humans = [r["num_human"] for r in self.results]

        plt.figure(figsize=(10, 6))
        scatter = plt.scatter(costs, profits, c=humans, cmap="viridis", s=100, alpha=0.8, edgecolors="black")
        cbar = plt.colorbar(scatter)
        cbar.set_label("Number of Human Servers")
        plt.title("Cost vs. Net Profit across all Simulation Runs")
        plt.xlabel("Total Daily Cost ($)")
        plt.ylabel("Daily Net Profit ($)")
        plt.grid(True, linestyle="--", alpha=0.7)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "cba_1_cost_vs_profit_scatter.png"))
        plt.close()

        # --- 2. Average Net Profit by Staffing Configuration ---
        config_profits = defaultdict(list)
        for r in self.results:
            label = f"{r['num_human']}H - {r['num_kiosk']}K"
            config_profits[label].append(r["net_profit"])

        # Calculate average profit for each layout
        avg_profits = {k: sum(v)/len(v) for k, v in config_profits.items()}
        # Sort from highest profit to lowest
        sorted_configs = sorted(avg_profits.items(), key=lambda x: x[1], reverse=True)

        labels = [x[0] for x in sorted_configs]
        values = [x[1] for x in sorted_configs]

        plt.figure(figsize=(12, 6))
        bars = plt.bar(labels, values, color="cornflowerblue", edgecolor="black")
        plt.title("Average Net Profit by Staffing Configuration")
        plt.xlabel("Configuration (Humans - Kiosks)")
        plt.ylabel("Average Net Profit ($)")
        plt.xticks(rotation=45, ha="right")
        plt.grid(axis="y", linestyle="--", alpha=0.7)

        # Highlight the most profitable configuration in green
        if bars:
            bars[0].set_color("mediumseagreen")
            bars[0].set_edgecolor("black")

        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "cba_2_avg_profit_by_config.png"))
        plt.close()

        # --- 3. Cost Breakdown vs Benefit (Top 10 Most Profitable Runs) ---
        # Sort raw results by net profit
        top_results = sorted(self.results, key=lambda x: x["net_profit"], reverse=True)[:10]

        top_labels = [f"{r['config_profile']}\n({r['num_human']}H-{r['num_kiosk']}K)" for r in top_results]
        top_costs = [r["cost"] for r in top_results]
        # Gross profit = Total Benefit * Margin
        top_gross = [r["benefit"] * r["profit_percentage"] for r in top_results]

        x = range(len(top_labels))

        plt.figure(figsize=(14, 7))
        plt.bar([i - 0.2 for i in x], top_costs, width=0.4, label="Total Cost", color="crimson")
        plt.bar([i + 0.2 for i in x], top_gross, width=0.4, label="Gross Profit (Benefit * Margin)", color="mediumseagreen")

        plt.xticks(x, top_labels, rotation=0)
        plt.ylabel("Amount ($)")
        plt.title("Cost vs Gross Profit (Top 10 Most Profitable Simulation Runs)")
        plt.legend()
        plt.grid(axis="y", linestyle="--", alpha=0.7)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "cba_3_cost_vs_benefit_top10.png"))
        plt.close()
        
        print(f"Cost-Benefit Analysis graphs exported to '{output_dir}'.")

    def generate_advanced_visualizations(self, output_dir="results"):
        """Generates advanced business intelligence representations."""
        if not self.results:
            return

        os.makedirs(output_dir, exist_ok=True)

        # --- 1. Profitability Heatmap (Humans vs Kiosks) ---
        humans = sorted(list(set(r["num_human"] for r in self.results)))
        kiosks = sorted(list(set(r["num_kiosk"] for r in self.results)))
        
        # Build a 2D list for the heatmap
        profit_matrix = []
        for h in humans:
            row = []
            for k in kiosks:
                profits = [r["net_profit"] for r in self.results if r["num_human"] == h and r["num_kiosk"] == k]
                row.append(sum(profits) / len(profits) if profits else 0)
            profit_matrix.append(row)

        fig, ax = plt.subplots(figsize=(8, 6))
        cax = ax.imshow(profit_matrix, cmap='RdYlGn', aspect='auto')
        fig.colorbar(cax, label="Avg Net Profit (₹)")
        
        ax.set_xticks(range(len(kiosks)))
        ax.set_xticklabels([f"{k} Kiosks" for k in kiosks])
        ax.set_yticks(range(len(humans)))
        ax.set_yticklabels([f"{h} Humans" for h in humans])
        
        # Add text annotations inside the heatmap blocks
        for i in range(len(humans)):
            for j in range(len(kiosks)):
                ax.text(j, i, f"₹{int(profit_matrix[i][j])}", ha="center", va="center", color="black", fontweight="bold")

        plt.title("Profitability Heatmap: Staffing Combinations", pad=15)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "adv_1_profit_heatmap.png"))
        plt.close()


        # --- 2. Customer Experience vs Cost Trade-off ---
        # Group by configuration to get average cost and wait time across all profiles
        config_summary = {}
        for r in self.results:
            label = f"{r['num_human']}H - {r['num_kiosk']}K"
            if label not in config_summary:
                config_summary[label] = {"costs": [], "waits": []}
            config_summary[label]["costs"].append(r["cost"])
            config_summary[label]["waits"].append(r["avg_wait_time_sec"] / 60) # Convert to minutes

        labels = list(config_summary.keys())
        avg_costs = [sum(c["costs"])/len(c["costs"]) for c in config_summary.values()]
        avg_waits = [sum(c["waits"])/len(c["waits"]) for c in config_summary.values()]

        plt.figure(figsize=(10, 6))
        plt.scatter(avg_costs, avg_waits, color='darkviolet', s=150, alpha=0.8, edgecolors='black')
        
        # Annotate points with their configuration label
        for i, txt in enumerate(labels):
            plt.annotate(txt, (avg_costs[i], avg_waits[i]), xytext=(8, -5), textcoords='offset points', fontsize=9)
            
        plt.title("Diminishing Returns: Customer Wait Time vs. Daily Infrastructure Cost")
        plt.xlabel("Total Daily Cost (₹)")
        plt.ylabel("Average Wait Time (Minutes)")
        plt.grid(True, linestyle="--", alpha=0.7)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "adv_2_cost_vs_wait_time.png"))
        plt.close()


        # --- 3. Opportunity Cost (Lost Revenue from Rejections) ---
        config_labels = []
        lost_revenue = []
        
        for r in self.results:
            # Derived average spend per customer for this specific run
            avg_spend = (r['benefit'] / r['customers_served']) if r['customers_served'] > 0 else 150
            # Lost Gross Profit = (Rejected Customers * Ticket Size) * Margin
            lost_profit = r['customers_not_served'] * avg_spend * r['profit_percentage']
            
            label = f"{r['config_profile']}\n({r['num_human']}H-{r['num_kiosk']}K)"
            config_labels.append(label)
            lost_revenue.append(lost_profit)

        # Sort to show the configurations bleeding the most money
        sorted_data = sorted(zip(config_labels, lost_revenue), key=lambda x: x[1], reverse=True)[:10] 
        
        if sorted_data:
            x_labels = [x[0] for x in sorted_data]
            y_vals = [x[1] for x in sorted_data]

            plt.figure(figsize=(12, 6))
            bars = plt.bar(x_labels, y_vals, color='tomato', edgecolor='black')
            plt.xticks(rotation=0)
            plt.title("Opportunity Cost: Lost Daily Profit Due to Max Queue Rejections (Top 10 Worst)")
            plt.ylabel("Lost Gross Profit (₹)")
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, "adv_3_lost_revenue_rejections.png"))
            plt.close()

        print(f"Advanced BI representations exported to '{output_dir}'.")
...
if __name__ == "__main__":
    ...
    # 1. Human Costs (Daily)
    # Salary: 15k/mo | Amortized Machine: 25k over 3 yrs | Maint: 10% of 25k/yr
    daily_human_salary = 15000 / 25 
    daily_human_machine = (25000 / 3) / 300
    daily_human_maint = (25000 * 0.10) / 300
    daily_kiosk_software = 2000 / 30 
    
    CALC_HUMAN_COST = daily_human_salary + daily_human_machine + daily_human_maint + daily_kiosk_software # ~529.68

    # 2. Kiosk Costs (Daily)
    # Amortized Machine: 75k over 3 yrs | Maint: 15% of 75k/yr
    daily_kiosk_machine = (75000 / 3) / 300
    daily_kiosk_maint = (75000 * 0.20) / 300
    CALC_KIOSK_COST = daily_kiosk_machine + daily_kiosk_maint + daily_kiosk_software # ~99.31

    # 3. Revenue & Overhead
    AVG_SPEND_PER_CUSTOMER = 120.0  # Adjust based on your cafeteria's average ticket size
    OVERHEAD_PERCENTAGE = 0.875      # 80% goes to food, rent, cooks, etc.
    PROFIT_MARGIN = 1.0 - OVERHEAD_PERCENTAGE # 20% margin
    
    print(f"--- Financial Parameters ---")
    print(f"Daily Cost per Human Server: ₹{CALC_HUMAN_COST:.2f}")
    print(f"Daily Cost per Kiosk: ₹{CALC_KIOSK_COST:.2f}")
    print(f"Gross Margin on Sales: {PROFIT_MARGIN*100}%")
    print(f"----------------------------\n")

    # Initialize tracker with our calculated profit margin
    tracker = BatchSimulationTracker(profit_percentage=PROFIT_MARGIN)
    ...
    for human_servers in range(1, 3):
        for kiosk_servers in range(0, 4):
            ...
            for config_no in range(1, 6):
                ...
                print(f"Running simulation with {config_name} - {human_servers}H / {kiosk_servers}K...")

                # Make sure the paths match your actual directory structure
                sim = CafeteriaSimulation(
                    profile_name=config_name,
                    arrivals_file="python_simulation/data/avg_day_arrivals.csv",
                    profiles_file="config/group_size_profiles.yaml",
                    num_human=human_servers,
                    num_kiosk=kiosk_servers,
                    human_cost=CALC_HUMAN_COST,           # Passing the new dynamic costs
                    kiosk_cost=CALC_KIOSK_COST,           # Passing the new dynamic costs
                    benefit_per_customer=AVG_SPEND_PER_CUSTOMER 
                )
                ...
                # Optional: Export individual sim logic (comment out if it creates too many files)
                sim.export_results(
                    output_dir=f"results/human{human_servers}_kiosk{kiosk_servers}_{config_name}"
                )

    tracker.export_overall_results("results")
    tracker.generate_cost_benefit_graphs("results")
    
    # Add this line to generate the new graphs!
    tracker.generate_advanced_visualizations("results")